 ...
 */
var _masterConfig = {};
var _masterDbh = null; // Connection pool
// If connection pool has been set up, MysqlDatabase will pick connections from it

var connectionPool = null; // Local dbh context for transaction. Each transaction generates its own local
...
var trxContext = _continuationLocalStorage.default.createNamespace('mysql-dbh');
/**
...
function () {
  ...
  _createClass(MysqlDatabase, [{
    ...
    value: function connectSync() {
      var future = new _future.default();
    ...
    value: function querySync(query, values) {
      var future = new _future.default(); // console.log("query", this._transacted?"(TRX)":"", query);
    ...
    value: function getRowSync(query, values) {
      var rows = this.querySync(query, values); // It is questionable: should we return {} or null below? Which is easier to use?
    ...
    key: "destroyPoll",
    value: function destroyPoll() {
      if (connectionPool) {
        connectionPool.end(function (err) {// console.log("connectionPool destroyed");
        });
      }
    }
  }]);
