...
class DbRecord {
  ...
  _read(locateValue, byKey = undefined, options = {}) {
    ...
    const rows = this._dbh.querySync(`SELECT * FROM ${this._tableName} WHERE ${field}=? LIMIT 1 ${forUpdate}`, [locateValue]);
  ...
  _readByKey(keys, values, options = {}) {
    ...
    const rows = this._dbh.querySync(`SELECT * FROM ${this._tableName} WHERE ${fields} LIMIT 1 ${forUpdate}`, values);
  ...
  static forEach(options, cb) {
    const forUpdate = options.forUpdate ? "FOR UPDATE" : "";
    let sql = `SELECT ${this._locatefield()} FROM ${this._table()} ${forUpdate}`;
    const where = [];
